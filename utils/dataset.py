...
class TABME(Dataset):
    """
    Args:
        path_data: path to folder containing images (.jpg file) and ocr results
        (.tsv file)
        path_cache: path to cache folder
        max_seq_length: will input only the first {max_seq_length} tokens into
        the model. Default: 100
        input_img_size: images are rescale into {input_img_size} before
    ...
    def __init__(
        ...
        if self.path_cache_folder:   # self.path_cache_folder != None
            ...
            self.path_cache = (
                path_cache_folder
                / f'cache_{self.path_data.stem}_{self.max_seq_length}.hdf5'
            )
            if not self.path_cache.exists():
                ...
                for page_id in tqdm(self.page_ids):
                    ...
                    (
                        ...
                        path_tsv, self.tokenizer, self.max_seq_length
                    )
                    group = self.hf.create_group(page_id)
                    group.create_dataset(
                        'input_ids', data=input_ids, compression='gzip'
                    )
                    ...
                    image = image_to_tensor(
                        path_img, input_img_size=self.input_img_size
                    )
                    group.create_dataset(
    ...
    def __getitem__(self, idx):
        ...
        attention_mask = torch.tensor(
            np.array(self.hf[page_id + '/attention_mask'])
        )
        token_type_ids = torch.tensor(
            np.array(self.hf[page_id + '/token_type_ids'])
        )
        ocr_input = {
            'input_ids': input_ids,
            'bbox': bbox,
...
def create_df(page_ids):
    pages = np.array(
        [
            (id.split('-') if len(id.split('-')) == 2 else [id, '0'])
            for id in page_ids
        ]
    )
...
def tokenize_from_ocr(path_tsv, tokenizer, max_seq_length):
    ...
    boxes = np.column_stack(
        (lefts, tops, widths + lefts, heights + tops)
    )   # [[x0, y0, x1, y1], ...]
    ...
    if len(token_boxes) <= max_seq_length:
        token_boxes = torch.tensor(
            np.array(
                token_boxes + (max_seq_length - len(token_boxes)) * [pad_box]
            )
        )
    else:
        token_boxes = torch.tensor(
            np.array(
                token_boxes[: max_seq_length - 1] + [[1000, 1000, 1000, 1000]]
            )
        )
