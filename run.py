...
from utils.train import get_validation_result, train
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'path_train',
        type=str,
        help='path to folder containing train set (.tsv and .jpg files), '
        "should be './data/train'",
    )
    parser.add_argument(
        'path_val',
        type=str,
        help='path to folder containing validation set (.tsv and .jpg files), '
        "should be './data/val'",
    )
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help='learning rate to run separated ex. --lr 1e-4, '
        "default=best from './output'",
    )
    parser.add_argument(
        '-n',
        '--num_hidden_layer',
        ...
        required=False,
        default=None,
        help='numbers of hidden layer to run ex. --num_hidden_layer 5, '
        "default=best from './output'",
    )
    parser.add_argument(
        '-o',
        ...
        required=False,
        default='./output',
        help='path to the outputs from validation, should be a folder '
    ...
    parser.add_argument(
        ...
        default=False,
        action='store_true',
        help='freeze layoutlm and resnet part',
    )
    parser.add_argument(
        '-w',
        '--load_weights',
        ...
        required=False,
        default=None,
        help='path to initialised weights, default=None '
        '(will use pretrained weights)',
    )
    ...
    if (learning_rate is None) or (num_hidden_layer is None):
        learning_rate, num_hidden_layer = get_validation_result(
            path_runs=path_runs
        )
    ...
    min_val_loss, min_val_f1 = train(
        ...
        path_model_weight,
        ablation=ablation,
        path_load_model_weights=load_weights,
        save_model=True,
        freeze=freeze,
    )

    print(
        f'Results: val_loss={min_val_loss}, '
        'val_f1={min_val_f1} saved in {path_model_weight}'
    )
