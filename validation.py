...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=float,
        nargs='+',
        help='learning rate(s) to run separated by space ex. '
        ' --lr 2.5e-5 5e-5 1e-4',
    )
    parser.add_argument(
        '-n',
        ...
        type=int,
        nargs='+',
        help='numbers of hidden layer(s) to run separated by '
        'space ex. -n 0 1 2 3',
    )
    parser.add_argument(
        '-o',
        ...
        required=False,
        default='./output',
        help="path to where to save the output csv file, default='./output'",
    )
    parser.add_argument(
        '-c',
        '--cache',
    ...
    if not Path(path_cache_folder).exists():
        Path(path_cache_folder).mkdir(parents=True, exist_ok=True)
    if not Path(path_runs).exists():
    ...
    for learning_rate in learning_rates:
        for num_hidden_layer in num_hidden_layers:
            ...
            num_hidden_features = [
                2 + spacing * (n + 1)
                for n in reversed(range(num_hidden_layer))
            ]
            ...
            results = results.append(
                {
                    ...
                    'num_features': num_hidden_features,
    ...
    print(
        f'Best results: learning_rate={best.learning_rate}, '
        'num_hidden_layer={best.num_hidden_layer}, saved in {path_csv}'
    )
